...
bool LedDeviceAdalight::init(const QJsonObject &deviceConfig)
{
	...
	if ( ProviderRs232::init(deviceConfig) )
	{
		...
		_ledBuffer[6] = _relayIsEnabled ? 0x01 : 0x00;

		Debug( _log, "Adalight header for %d leds: %c%c%c 0x%02x 0x%02x 0x%02x 0x%02x", _ledCount,
			   _ledBuffer[0], _ledBuffer[1], _ledBuffer[2], _ledBuffer[3], _ledBuffer[4], _ledBuffer[5], _ledBuffer[6] );
...
bool LedDeviceAdalight::powerOn()
{
	_relayIsEnabled = true;
	return ProviderRs232::powerOn();
}

bool LedDeviceAdalight::powerOff()
{
	_relayIsEnabled = false;
	return ProviderRs232::powerOff();
}
